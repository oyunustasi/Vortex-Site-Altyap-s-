...
module.exports = async client => {
      ...
      if (req.isAuthenticated()) {
        ...
        if (usercheck) {
          if (usercheck.roles.cache.get(roles.yonetici)) {
            ...
            res.redirect(
              "/error?code=200&message=Sitemiz geçici olarak kullanılamıyor."
            );
          }
        } else {
          res.redirect(
  ...
  app.get(
    ...
    passport.authenticate("discord", {
      failureRedirect:
        "/error?code=999&message=Bağlanırken bir hatayla karşılaştık."
    }),
    async (req, res) => {
        ...
        client.users.fetch(req.user.id).then(async a => {
          client.channels.cache.get(channels.login).send(
            new Discord.MessageEmbed()
              ...
              .setColor("RED")
              .setDescription(
                `[${a.username}#${a.discriminator}](https://vortex-botlistcodeshare.glitch.me/user/${a.id}) isimli kullanıcı siteye giriş yapmaya çalıştı fakat siteden engellendiği için giriş yapamadı.`
              )
              .addField("Kullanıcı Adı", a.username)
              .addField("Kullanıcı ID", a.id)
        ...
        client.users.fetch(req.user.id).then(async a => {
          client.channels.cache.get(channels.login).send(
            new Discord.MessageEmbed()
              ...
              .setColor("GREEN")
              .setDescription(
                `[${a.username}#${a.discriminator}](https://vortex-botlistcodeshare.glitch.me/user/${a.id}) isimli kullanıcı siteye giriş yaptı.`
              )
              .addField("Kullanıcı Adı", a.username)
              .addField("Kullanıcı ID", a.id)
              .addField("Kullanıcı Etiket", a.discriminator)
  ...
  app.get("/code/:code", checkMaintence, checkAuth, async (req, res) => {
    ...
    if (!koddata)
      return res.redirect("/codes?error=true&message=Kod geçersiz.");
    if (
    ...
    if (koddata.codeCategory == "javascript") {
      if (
        ...
        return res.redirect(
          "/error?code=403&message=Bunu yapmaya yetkili değilsin."
        );
    }
    ...
    if (koddata.codeCategory == "subs") {
      if (
        !client.guilds.cache
          .get(config.server.id)
          .members.cache.get(req.user.id)
          .roles.cache.get(config.server.roles.codeshare.altyapilar)
      )
        return res.redirect(
          "/error?code=403&message=Bunu yapmaya yetkili değilsin."
        );
    ...
    if (koddata.codeCategory == "5invites") {
      if (
        !client.guilds.cache
          .get(config.server.id)
          .members.cache.get(req.user.id)
          .roles.cache.get(config.server.roles.codeshare.besdavet)
      )
        return res.redirect(
          "/error?code=403&message=Bunu yapmaya yetkili değilsin."
        );
    ...
    if (koddata.codeCategory == "10invites") {
      if (
        !client.guilds.cache
          .get(config.server.id)
          .members.cache.get(req.user.id)
          .roles.cache.get(config.server.roles.codeshare.ondavet)
      )
        return res.redirect(
          "/error?code=403&message=Bunu yapmaya yetkili değilsin."
        );
    ...
    if (koddata.codeCategory == "15invites") {
      if (
        !client.guilds.cache
          .get(config.server.id)
          .members.cache.get(req.user.id)
          .roles.cache.get(config.server.roles.codeshare.onbesdavet)
      )
        return res.redirect(
          "/error?code=403&message=Bunu yapmaya yetkili değilsin."
        );
    ...
    if (koddata.codeCategory == "20invites") {
      if (
        !client.guilds.cache
          .get(config.server.id)
          .members.cache.get(req.user.id)
          .roles.cache.get(config.server.roles.codeshare.yirmidavet)
      )
        return res.redirect(
          "/error?code=403&message=Bunu yapmaya yetkili değilsin."
        );
    ...
    if (koddata.codeCategory == "bdfd") {
      if (
        !client.guilds.cache
          .get(config.server.id)
          .members.cache.get(req.user.id)
          .roles.cache.get(rconfig.server.roles.codeshare.bdfd)
      )
        return res.redirect(
          "/error?code=403&message=Bunu yapmaya yetkili değilsin."
        );
  ...
  app.post("/uptime/add", checkMaintence, checkAuth, async (req, res) => {
    ...
    if (!rBody["link"]) {
      ...
      if (!rBody["link"].match("https"))
        return res.redirect("?error=true&message=Geçerli link girmelisin.");
  ...
  app.get(
    ...
    async (req, res) => {
      ...
      if (!dde)
        return res.redirect(
          "/uptime/links?error=true&message=Böyle bir site yok."
        );
      uptimedata.findOne(
        {
          code: req.params.code
        ...
        async function(err, docs) {
          if (docs.userID != req.user.id)
            return res.redirect(
              "/uptime/links?error=true&message=Eklenmiş linklerden birini silmelisin."
            );
          res.redirect(
            "/uptime/links?success=true&message=Link başarıyla sistemden silindi."
          );
  ...
  app.post("/bot/:botID/vote", checkMaintence, checkAuth, async (req, res) => {
    ...
    if (x)
      return res.redirect(
        "/error?code=400&message=12 saate bir defa oy verebilirsin."
      );
    ...
    client.channels.cache
      ...
      .send(
        `${req.user.username} adlı kullanıcı ${
          botdata.username
        } adlı bota oy verdi. (${botdata.votes + 1} oy)`
      );
    return res.redirect(
      `/bot/${req.params.botID}/vote?success=true&message=Oy başarıyla verildi. 12 saat sonra tekrar oy verebilirsin.`
    );
    renderTemplate(res, req, "botlist/vote.ejs", {
      req,
      roles,
  ...
  app.post("/addbot", checkMaintence, checkAuth, async (req, res) => {
    ...
    client.users.fetch(req.body.botID).then(async a => {
      ...
      if (rBody["coowners"]) {
        if (String(rBody["coowners"]).split(",").length > 3)
          return res.redirect(
            "?error=true&message=3 tane sahip ekleyebilirsiniz."
          );
        if (
          ...
          return res.redirect(
            "?error=true&message=Diğer sahiplere kendini ekleyemezsin"
          );
      }
      await new botsdata({
        botID: rBody["botID"],
      ...
      if (rBody["coowners"]) {
        if (String(rBody["coowners"]).split(",").length > 3)
          return res.redirect(
            "?error=true&message=3 tane sahip ekleyebilirsin."
          );
        if (
          String(rBody["coowners"])
            .split(",")
          ...
          return res.redirect(
            "?error=true&message=Kendini diğer sahiplere ekleyemezsin."
          );
        await botsdata.findOneAndUpdate(
          {
            botID: rBody["botID"]
    ...
    client.users.fetch(rBody["botID"]).then(a => {
      client.channels.cache
        .get(channels.botlog)
        .send(`<@${req.user.id}> bot eklendi **${a.tag}**`);
      res.redirect(
        `?success=true&message=Bot sisteme başarıyla .&botID=${rBody["botID"]}`
      );
    });
  });
  ...
  app.get("/bot/:botID", checkMaintence, async (req, res, next) => {
    ...
    if (botdata.status != "Approved") {
      if (
        ...
        res.redirect(
          "/error?code=404&message=Botu sadece sahibi düzenleyebilir"
        );
      }
    } else {
      let coowner = new Array();
  ...
  app.post("/bot/:botID/edit", checkMaintence, checkAuth, async (req, res) => {
    ...
    if (String(rBody["coowners"]).split(",").length > 3)
      return res.redirect("?error=true&message=3 tane sahip ekleyebilirsiniz.");
    if (
      String(rBody["coowners"])
        .split(",")
        .includes(req.user.id)
      ...
      return res.redirect(
        "?error=true&message=Kendini diğer sahiplere ekleyemezsiniz."
      );
    await botsdata.findOneAndUpdate(
      {
        botID: req.params.botID
  ...
  app.get("/bot/:botID/delete", async (req, res) => {
    ...
    if (
      ...
      return res.redirect(
        `/user/${req.user.id}?success=true&message=Bot başarıyla silindi.`
      );
    } else {
      return res.redirect("/error?code=404&message=Bot id geçersiz.");
    }
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      client.channels.cache
        .get(channels.botlog)
        .send(
          `${req.user.username} adlı kullanıcı ${botdata.username} adlı bot için sertifika isteğinde bulundu.`
        );
    }
  );
  //
    ...
    async (req, res) => {
      ...
      client.users.fetch(req.params.botID).then(bota => {
        client.channels.cache
          .get(channels.botlog)
          .send(
            `<@${botdata.ownerID}> adlı kullanıcının ${bota.tag} adlı botu onaylandı. `
          );
        client.users.cache
          .get(botdata.ownerID)
          .send(`${bota.tag} adlı bot onaylandı.`);
      });
      let guild = client.guilds.cache.get(config.server.id);
      guild.members.cache.get(botdata.botID).roles.add(roles.botlist.bot);
      guild.members.cache
        .get(botdata.ownerID)
        .roles.add(roles.botlist.developer);
      if (botdata.coowners) {
        botdata.coowners.map(a => {
    ...
    async (req, res) => {
      ...
      client.users.fetch(botdata.ownerID).then(sahip => {
        client.channels.cache
          .get(channels.botlog)
          .send(
            `<@${botdata.ownerID}> adlı kullanıcının ${botdata.username} adlı botu reddedildi. `
          );
        client.users.cache
          .get(botdata.ownerID)
          .send(
            `${botdata.username} adlı bot reddedildi.\nSebep: **${
              rBody["reason"]
    ...
    async (req, res) => {
      ...
      client.users.fetch(botdata.botID).then(bota => {
        client.channels.cache
          .get(channels.botlog)
          .send(
            `<@${botdata.ownerID}> adlı kullanıcının ${bota.tag} adlı botuna sertifika verildi`
          );
        client.users.cache
          .get(botdata.ownerID)
          .send(`${bota.tag} adlı bota sertifika verildi.`);
      });
      ...
      return res.redirect(
        `/admin/certificate-apps?success=true&message=Sertifika verildi.&botID=${req.params.botID}`
      );
    }
  );
  app.get(
    ...
    async (req, res) => {
      ...
      client.users.fetch(botdata.botID).then(bota => {
        client.channels.cache
          .get(channels.botlog)
          .send(
            `<@${botdata.ownerID}> adlı kullanıcının ${bota.tag} adlı botuna sertifika verilmedi.`
          );
        client.users.cache
          .get(botdata.ownerID)
          .send(
            `${bota.tag} adlı bota sertifika verilmedi.\nSebep: **${
              rBody["reason"]
            }**`
          );
      });
    ...
    async (req, res) => {
      ...
      client.channels.cache.get(channels.codelog).send(
        new Discord.MessageEmbed()
          ...
          .addField(
            "Kod Link",
            `https://vortex-botlistcodeshare.glitch.me/code/${kod}`,
            true
          )
        ...
        new Discord.MessageEmbed()
          ...
          .setDescription(
            `[${
              ...
              req.user.id
            }) adlı kullanıcı **${rBody["codename"]}** adlı kodu düzenledi.`
          )
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      client.channels.cache
        ...
        .send(`site bakımda **${req.body.reason}**`)
        .then(a => {
          new maintenceSchema({
            server: config.server.id,
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      if (!bakimdata)
        return res.redirect(
          "../admin/maintence?error=true&message=Zaten bakımda."
        );
      ...
      return res.redirect(
        "../admin/maintence?success=true&message=Başarıyla bakım moduna geçildi."
      );
    }
  );
  app.get(
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      return res.redirect(
        "../admin/userban?success=true&message=Kullanıcı yasaklandı"
      );
    }
  );
  app.post(
    ...
    async (req, res) => {
      ...
      return res.redirect(
        "../admin/userban?success=true&message=Kullanıcının yasağı kalktı."
      );
    }
  );
